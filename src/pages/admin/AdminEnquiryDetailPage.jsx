...
function qtyLabel(value) {
  ...
  if (!text) return ''
  if (/pcs|piece|kg|g\b/i.test(text)) return text
  return `${text} pcs`
}
  ...
  return (
    <section className="page admin-page admin-enquiry-detail">
      ...
      <article className={`job-hero job-card--${statusTone(savedStatus)}`}>
        ...
        {pretty(enquiry.shape) || pretty(enquiry.occasion || enquiry.occasionOther) || pretty(enquiry.theme) ? (
          <div className="job-block">
            <Fact icon={Shapes} label="Shape" value={pretty(enquiry.shape)} />
            <Fact icon={PartyPopper} label="Occasion" value={pretty(enquiry.occasion || enquiry.occasionOther)} />
        ...
        <div className="job-block">
          ...
          />
          {pretty(enquiry.fulfillmentType) === 'Delivery' && (address?.address || address?.area) ? (
            <Fact
              icon={MapPin}
              label="Address"
              value={`${[address.address, address.area, address.pincode].filter(Boolean).join(', ')}${
        ...
        {pretty(enquiry.otherRequirements) || pretty(enquiry.referenceNotes) ? (
          <div className="job-block">
            <Fact
              icon={StickyNote}
              label="Note"
              value={[pretty(enquiry.otherRequirements), pretty(enquiry.referenceNotes)].filter(Boolean).join(' · ')}
            />
          </div>
        ) : null}
